...
void World::init(ResourceManager *rm)
{
    ...
    rm->LoadTexture("player", "res/graphics/player.png");

    // Create World
    std::shared_ptr<GameObject> go = std::make_shared<GameObject>();
    
    go->AddComponent(std::make_shared<Transform>(100, 100));
    go->AddComponent(std::make_shared<Sprite>(rm->GetTexture("player")));
